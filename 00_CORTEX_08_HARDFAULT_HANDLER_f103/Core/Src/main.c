...
__STATIC_INLINE void undef(void)
{ // .word is not an ARM assembly instruction
  __ASM(".word  0xF123\n"
  		".word  0x4567\n"
  		"bx lr"
  		);
 ...
 */
  ...
  #define CHECK_UNALIGN
	...
	//#define CHECK_DIVBYZERO
	#ifdef CHECK_DIVBYZERO
    SCB->CCR = SCB->CCR | 0x210;                     // enable div-by-zero trap in processor core
	#endif

	#define ENABLE_USAGE_MEM_BUS
	#ifdef ENABLE_USAGE_MEM_BUS
